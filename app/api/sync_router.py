...
from app.core.story_sync_service import sync_stories, rebuild_dataset, clear_dataset, get_user_llm_metadata, get_user_llm_context, sync_stories_for_senior # sync_stories_for_senior 임포트
import logging
...
@router.post("/stories/senior")
async def trigger_story_sync_for_senior(senior_user_id: int, db: Session = Depends(get_db)):
    """특정 시니어 사용자의 story-sequencer 데이터 동기화를 트리거합니다."""
    logger.info(f"Manual story synchronization triggered for senior_user_id: {senior_user_id}.")
    try:
        await sync_stories_for_senior(db, senior_user_id)
        return {"message": f"Story synchronization initiated successfully for senior_user_id {senior_user_id}.", "status": "success"}
    except Exception as e:
        logger.error(f"Error during manual story synchronization for senior_user_id {senior_user_id}: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Synchronization failed for senior_user_id {senior_user_id}: {e}")
